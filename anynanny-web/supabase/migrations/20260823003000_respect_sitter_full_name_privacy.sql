-- F20: public sitter RPCs must respect sitter_profiles.show_full_name.
-- last_name is kept as a JSON key but is null when the sitter hides their full name.
-- show_full_name itself is never returned.
--
-- Preserves F7 projection, onboarding eligibility, and profiles.avatar_url source.
-- Does NOT recreate the obsolete 12-arg list_public_sitters_search overload.
-- Does NOT modify sitter_profiles RLS.
-- Does NOT touch booking lifecycle cron or the expired-pending approval trigger.

-- ---------------------------------------------------------------------------
-- 1. get_sitter_profile_public — same F7 fields; last_name gated by show_full_name.
-- ---------------------------------------------------------------------------
...
comment on function public.get_sitter_profile_public(uuid) is
  'Public sitter profile JSON. last_name/display_name respect show_full_name. avatar_url prefers public.profiles.avatar_url.';
...
-- ---------------------------------------------------------------------------
-- 2. list_public_sitters_search — same 9-arg F7 projection; name privacy applied.
-- ---------------------------------------------------------------------------
...
comment on function public.list_public_sitters_search(
  ...
  'Public sitter discovery. last_name/display_name respect show_full_name. avatar_url prefers public.profiles.avatar_url.';
