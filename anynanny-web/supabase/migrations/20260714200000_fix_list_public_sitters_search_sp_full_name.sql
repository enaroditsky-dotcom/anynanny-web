-- Fix live list_public_sitters_search: was selecting p.full_name (profiles) which no longer exists.
-- Correct source is sp.full_name (sitter_profiles); profiles fallback uses first_name + last_name.
-- Also restore browse filters (apply when NOT serial-targeted).
...
as $$
  ...
  where coalesce(sp.is_public, false) = true
    ...
    -- Browse mode (no serial): apply experience / rating / transport / rate / city filters.
    -- Serial lookup: skip those filters so exact AN-#### always returns.
    and (
