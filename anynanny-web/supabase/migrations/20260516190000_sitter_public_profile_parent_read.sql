-- Parents (authenticated) can read public sitter_profiles for marketplace profile pages.

drop policy if exists "sitter_profiles_select_public_authenticated" on public.sitter_profiles;

create policy "sitter_profiles_select_public_authenticated"
  on public.sitter_profiles
  for select
  to authenticated
  using (coalesce(is_public, false) = true);

-- Include full_name + nanny_serial in public profile RPC JSON.
