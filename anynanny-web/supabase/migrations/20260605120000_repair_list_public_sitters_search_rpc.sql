-- Repair / (re)create public.list_public_sitters_search so the parent search view
-- stops crashing on re-hydration. Non-destructive: only the function is dropped/recreated
-- (no table/data changes). Output JSON shape matches the TypeScript `PublicSitterSearchCard`
-- type (lib/sitter/sitter-profile.ts) and `normalizePublicSearchCard` (lib/sitter/public-search-card.ts).

-- Drop any stale overloads first (signature/return-type drift requires a real drop, not just replace).
do $$
declare
  r record;
begin
