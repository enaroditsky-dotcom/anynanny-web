-- Align profiles name columns with first_name / last_name (full_name removed).
-- Updates auth signup trigger + search RPC fallback that still referenced profiles.full_name.
...
-- Prefer dropping the legacy column only if it still exists (idempotent for projects that already dropped it).
do $$
begin
  if exists (
    select 1
...
-- Recreate list_public_sitters_search: profiles fallback uses first_name + last_name.
do $$
declare
  r record;
begin
