-- Add minimum average rating filter to parent sitter search.

drop function if exists public.list_public_sitters_search(text, timestamptz, int, text, numeric);

create or replace function public.list_public_sitters_search(
  p_search_nanny_id text default null,
  p_search_date timestamptz default null,
...
as $$
  with filters as (
    select
      nullif(trim(p_search_nanny_id), '') as nanny_q,
      p_search_date as need_at,
  ...
  where coalesce(sp.is_public, false) = true
    and (
      f.nanny_q is null
      or sp.nanny_serial ilike '%' || f.nanny_q || '%'
    )
    ...
    and (
      ...
      or not exists (
        ...
        where s.sitter_id = sp.id
          ...
          and s.start_time <= f.need_at
          and coalesce(s.end_time, s.start_time + interval '4 hours') > f.need_at
      )
    );
$$;

grant execute on function public.list_public_sitters_search(text, timestamptz, int, numeric, text, numeric) to authenticated;
