-- Parent-facing identity badge: expose a display-only boolean derived from
...
-- Does not change RLS, table grants, or the verified-only search filter.
...
begin
  ...
  select coalesce(pr.identity_verification_status, 'unverified') = 'verified'
    into identity_ok
  from public.profiles pr
  where pr.id = target_id;

  identity_ok := coalesce(identity_ok, false);
  ...
  return jsonb_build_object(
    ...
    'avatar_url', photo,
    'identity_verified', coalesce(identity_ok, false),
...
comment on function public.get_sitter_profile_public(uuid) is
  'Public sitter profile JSON. Includes preferred receiving-method enum and identity_verified boolean derived from profiles.identity_verification_status. Hides suspended and mutually blocked sitters. last_name/display_name respect show_full_name.';
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'identity_verified', (coalesce(p.identity_verification_status, 'unverified') = 'verified')
      )
...
comment on function public.list_public_sitters_search(
  ...
  'Public sitter discovery. Optional p_verified_only AND-filters profiles.identity_verification_status = verified. Returns identity_verified boolean only; does not expose identity documents, ID numbers, or KYC metadata.';
