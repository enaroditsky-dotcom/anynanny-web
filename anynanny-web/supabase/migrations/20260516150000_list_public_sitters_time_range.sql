-- Parent search: availability window via p_start_time / p_end_time (replaces single p_search_date).

drop function if exists public.list_public_sitters_search(text, timestamptz, int, text, numeric);
drop function if exists public.list_public_sitters_search(text, timestamptz, int, numeric, text, numeric);
