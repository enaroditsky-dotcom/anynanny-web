-- Temporary pause: sitters hidden from parent search while keeping profile/onboarding intact.

alter table public.sitter_profiles
  add column if not exists is_paused boolean not null default false;

comment on column public.sitter_profiles.is_paused is
  'When true, sitter is temporarily hidden from parent search (profile remains active).';

-- Recreate list_public_sitters_search to exclude paused sitters.
do $$
declare
  r record;
begin
