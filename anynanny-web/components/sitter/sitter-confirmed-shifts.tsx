    ...
    >
      <div className="flex flex-row-reverse items-start justify-between gap-2">
        <span
          className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold ${
            isPast ? "bg-slate-200 text-slate-700" : "bg-emerald-100 text-emerald-900"
          }`}
        >
          {isPast ? "הסתיימה" : "קרובה"}
        </span>
      ...
      <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
        <button
          type="button"
          disabled
          title="אימות התחלת משמרת — יופעל בקרוב"
  ...
  return (
    <section className="mx-1 space-y-4">
      <p className="rounded-2xl border border-navy-header/10 bg-white px-4 py-3 text-right text-xs text-slate-600 shadow-sm">
        מוצגות משמרות <span className="font-semibold text-[#001F3F]">מאושרות בלבד</span> מטבלת הבקשות — לא זמינות בסיס.
      </p>
