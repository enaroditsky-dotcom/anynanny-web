    ...
    <div className="flex w-full shrink-0 flex-col items-center">
      <div className="w-full max-w-[16rem] rounded-xl bg-[#001F3F] px-2.5 py-2 shadow-[0_6px_20px_-6px_rgba(0,31,63,0.5)] ring-1 ring-[#001F3F]/20">
        <div className="flex flex-col items-center gap-1 text-center">
          ...
          <p className="mb-0.5 text-[11px] font-medium text-white/75">הדירוג עוזר לקהילה</p>

          <StarRatingInput value={rating} onChange={setRating} disabled={busy} size="sm" />

          <label className="mt-1 w-full text-right">
            <span className="mb-0.5 block text-[11px] font-semibold text-white/80">
              חוות דעת (לא חובה)
            </span>
            <textarea
              value={comment}
              ...
              className="w-full resize-none rounded-lg border border-white/15 bg-white/10 px-2 py-1.5 text-[12px] leading-snug text-white placeholder:text-white/45 focus:outline-none focus:ring-1 focus:ring-emerald-400/60"
            />
          </label>

          {errorMessage ? (
            <p className="max-w-full text-[11px] font-medium leading-snug text-rose-300">
              {errorMessage}
            </p>
          ) : null}
