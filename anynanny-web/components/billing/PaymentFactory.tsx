...
const PAYMENT_OPTIONS: {
  ...
  { id: "credit_card", label: "כרטיס אשראי", icon: CreditCard },
  { id: "bit", label: "Bit", icon: Smartphone },
  { id: "paybox", label: "Paybox", icon: Wallet }
];
...
/** Shift settlement payment picker — credit card, Bit, or Paybox. */
export function PaymentFactory({
  ...
  const canConfirm =
    Boolean(selectedMethod) && !busy && bookingReady && !bookingChecking;
    ...
    <div className="flex w-full shrink-0 flex-col items-center">
      <div className="w-full max-w-[15rem] rounded-xl bg-[#001F3F] px-2.5 py-2 shadow-[0_6px_20px_-6px_rgba(0,31,63,0.5)] ring-1 ring-[#001F3F]/20">
        <div className="flex flex-col items-center gap-1.5 text-center">
          ...
          >
            {busy ? "מעבדים תשלום…" : "אישור ותשלום"}
          </button>
        </div>
      </div>
    </div>
