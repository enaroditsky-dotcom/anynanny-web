...
import { readStrictSessionStatus, resolveBillingLifecyclePhase } from "@/lib/billing/billing-lifecycle";
...
export function ParentActiveSession({ sessionId, parentId, className = "" }: ParentActiveSessionProps) {
  ...
  const handleParentConfirmEnd = useCallback(async () => {
    ...
    if (confirmEndBusy || sessionStatus !== "sitter_ended" || !row) return;
  ...
  }, [commitSessionRow, confirmEndBusy, parentId, row, sessionId, sessionStatus]);
  ...
  } else if (sessionStatus === "in_progress" && !genuinelyCompleted) {
