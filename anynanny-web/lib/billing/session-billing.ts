...
/** Parent-only: kick off Stripe once the DB row reaches COMPLETED (never from local phase state). */
export function useBillingStripeOnCompleted(params: {
  ...
  const { row, billingState, completedSummary, sessionId, parentId } = params;
  const [stripeProcessing, setStripeProcessing] = useState(false);
  const [stripeComplete, setStripeComplete] = useState(false);
  const [stripeError, setStripeError] = useState<string | null>(null);
  const startedForEndShakeRef = useRef<string | null>(null);

  useEffect(() => {
    if (billingState !== "COMPLETED" || !row?.parent_end_shake || !completedSummary) return;
    if (startedForEndShakeRef.current === row.parent_end_shake) return;

    if (row.session_status === "paid" || row.stripe_payment_intent_id) {
      setStripeComplete(true);
      setStripeProcessing(false);
      startedForEndShakeRef.current = row.parent_end_shake;
      return;
    }
    ...
    void (async () => {
      ...
      const { postStripePaymentIntent } = await import("@/lib/stripe/post-payment-intent");
      const payment = await postStripePaymentIntent({ sessionId });

      if (!payment.ok) {
        setStripeError(payment.error);
        setStripeProcessing(false);
        startedForEndShakeRef.current = null;
        return;
      }
      ...
      // PaymentIntent created server-side from DB shakes; mark paid when webhook confirms.
      // Until Stripe.js Elements are wired, treat intent creation as checkout handoff complete.
      if (payment.clientSecret) {
        const supabase = getSupabaseBrowserClient();
        if (supabase && payment.paymentIntentId) {
          await supabase
            .from(SESSIONS_TABLE)
            .update({
              stripe_payment_intent_id: payment.paymentIntentId,
              total_amount_charged: payment.amount ?? completedSummary.amountNis
            })
            .eq("id", sessionId)
  ...
  }, [billingState, completedSummary, parentId, row, sessionId]);

  return { stripeProcessing, stripeComplete, stripeError };
}
