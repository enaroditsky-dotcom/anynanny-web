      ...
      <section className="mt-1 flex min-h-0 flex-1 flex-col items-center rounded-3xl border-2 border-[#001F3F]/20 bg-white p-4 shadow-[0_16px_48px_-12px_rgba(0,31,63,0.45)] sm:p-6">
        {sessionRunning ? (
          <div className="w-full space-y-2 text-right">
            ...
            {(sessionState.status === "active" || waitingParentEndConfirm) && (
              <>
        ...
        {sessionState.status === "ended" ? (
          <div className="w-full space-y-1 text-right">
            ...
            <p className="text-sm font-semibold text-navy-800">סה״כ: ₪{earnedNis}</p>
          </div>
        ) : null}
        ...
        <div className="mt-auto flex w-full flex-1 flex-col items-center justify-center gap-4 pt-8">
          {!sessionRunning ? (
            <button
              ...
              className={`${circleShell} gap-1 bg-[#001F3F] shadow-[0_12px_40px_-10px_rgba(0,31,63,0.65)] ring-[#001F3F]/25 transition hover:brightness-110 active:brightness-95 disabled:opacity-80`}
            >
              <span className="max-w-[13rem]">התחל משמרת</span>
            ...
            <button
              ...
              className={`${circleShell} gap-1 bg-emerald-600 shadow-[0_12px_32px_-10px_rgba(5,150,105,0.55)] ring-emerald-700/25 transition hover:brightness-105 active:brightness-95 disabled:opacity-80`}
            >
              <span className="max-w-[13rem]">אשר סיום משמרת</span>
            </button>
            ...
            <button
              ...
              className={`${circleShell} gap-1 bg-[#FF8A8A] shadow-[0_10px_36px_-8px_rgba(255,138,138,0.75)] ring-[#FF8A8A]/40 transition hover:brightness-105 active:brightness-95 disabled:opacity-80`}
            >
