...
export default function ParentSitterProfilePage() {
  ...
  const raw = params?.sitterId;
  const sitterId = decodeURIComponent(Array.isArray(raw) ? (raw[0] ?? "") : String(raw ?? "")).trim();
  ...
  useEffect(() => {
    if (isLoading) return;
    if (!signedIn) {
      const next = sitterId ? `/parent/sitter/${sitterId}` : "/parent/sitter";
  ...
  const loadProfile = useCallback(async () => {
    ...
    if (profErr) {
      console.error("[parent sitter profile]", profErr.message);
      setPageState("missing");
      return;
    }
    if (profileJson == null) {
    ...
    setProfile(profileJson as SitterProfilePublic);
  ...
  useEffect(() => {
    ...
    void loadProfile();
  }, [isLoading, signedIn, effectiveRole, sitterId, loadProfile]);
