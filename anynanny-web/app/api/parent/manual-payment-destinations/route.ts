import { loadAuthorizedManualPaymentDestinations } from "@/lib/billing/parent-manual-payment-server";
...
export async function GET(request: Request) {
  ...
  const bookingId = parsePaymentBookingIdParam(
  ...
  const result = await loadAuthorizedManualPaymentDestinations(supabase, {
    actorId: user.id,
    bookingId
  });
