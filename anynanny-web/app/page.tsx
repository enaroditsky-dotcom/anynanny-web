...
import { Baby, Droplet, MoonStar, Users } from "lucide-react";
import { setUserRoleChoice } from "@/lib/auth/returning-user";
import { DoulaIcon } from "@/components/sitter/expert-service-icons";

type LandingPath = "parent" | "sitter" | "expert";

function ExpertClusterIcon({ className = "h-6 w-6" }: { className?: string }) {
  return (
    <span className={`relative inline-flex items-center justify-center ${className}`} aria-hidden>
      <Droplet className="absolute -translate-x-2.5 -translate-y-1 h-[55%] w-[55%] text-emerald-600" />
      <MoonStar className="absolute translate-x-2.5 -translate-y-1 h-[55%] w-[55%] text-indigo-600" />
      <DoulaIcon className="absolute translate-y-2 h-[58%] w-[58%] text-rose-500" />
    </span>
  );
}
...
function PathIcon({ path }: { path: LandingPath }) {
  ...
  if (path === "sitter") return <Baby className="h-6 w-6 stroke-[1.75]" aria-hidden />;
  return <ExpertClusterIcon className="h-7 w-7" />;
}
...
const PATHS: Array<{
...
}> = [
  {
    id: "parent",
    title: "הורים",
    subtitle: "חיפוש בייביסיטר ויועצות",
  ...
  },
  {
    id: "expert",
    title: "יועצת / דולה",
    subtitle: "הנקה · שינה · דולה",
    accent: "border-emerald-300/50 bg-gradient-to-b from-emerald-50 to-rose-50 text-emerald-950",
    iconWrap: "bg-white text-emerald-700 ring-emerald-200/70"
  }
];
...
function HomeInner() {
  ...
  const navigateWithPath = (action: "login" | "register", path: LandingPath) => {
    ...
    try {
      localStorage.setItem("anynanny_service_track", path === "expert" ? "expert" : path === "sitter" ? "babysitter" : "parent");
    } catch {
      /* ignore */
    }
    const qs = new URLSearchParams({ role: profileRole });
    if (path === "expert") qs.set("track", "expert");
    if (path === "sitter") qs.set("track", "babysitter");
      ...
      <div className="mx-auto flex h-full w-full max-w-md flex-col items-center justify-between gap-2 px-3 py-2.5 text-center sm:justify-center sm:gap-4 sm:py-5">
        ...
        <div className="flex shrink-0 justify-center">
          <div className="relative flex h-[min(28vh,200px)] w-[min(28vh,200px)] shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-navy-header/20 bg-white shadow-md sm:h-[240px] sm:w-[240px]">
            {/* Same circular nanny mark as AppShellHeader — enlarged for landing hero */}
            <img
          ...
          <div className="mt-2.5 grid grid-cols-3 gap-1.5 sm:mt-4 sm:gap-2">
            {PATHS.map((path) => (
              <button
                ...
                className={`flex flex-col items-center gap-1 rounded-xl border px-1 py-2 text-center transition hover:brightness-[0.99] active:scale-[0.98] sm:gap-2 sm:rounded-2xl sm:px-1.5 sm:py-3 ${path.accent}`}
              >
                <span
          ...
          <div className="mt-2 grid grid-cols-3 gap-1.5 sm:mt-3 sm:gap-2.5">
            {PATHS.map((path) => (
              <button
                ...
                className={`flex flex-col items-center gap-1 rounded-xl border bg-white px-1 py-2 text-center shadow-sm transition hover:bg-slate-50 active:scale-[0.98] sm:gap-2 sm:rounded-2xl sm:px-2.5 sm:py-3.5 ${path.accent}`}
              >
                <span
                ...
                <span className="min-w-0 space-y-0.5">
                  <span className="block text-[10px] font-bold leading-tight sm:text-xs">{path.title}</span>
                  <span className="block text-[9px] font-medium leading-snug opacity-75 sm:text-[10px]">
                    הרשמה
                  </span>
                </span>
              </button>
        ...
        <p className="max-w-sm shrink-0 text-pretty text-[10px] leading-snug text-slate-500 sm:text-sm">
          הורים, בייביסיטריות ויועצות מקצועיות — כל הקהילה במקום אחד.
        </p>
      </div>
    </main>
  );
