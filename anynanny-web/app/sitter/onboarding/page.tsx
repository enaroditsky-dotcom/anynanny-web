...
export default function SitterOnboardingPage() {
  ...
  const validateFinalize = (skipEmailPassword: boolean): string | null => {
    const complete = isSitterProfileComplete({ ...draftPayload, id: "" } as SitterProfileRow);
    if (!complete) return "חסרים שדות חובה — עברו על כל השלבים.";
    if (!legal_no_criminal_declaration) return "יש לאשר את ההצהרה המשפטית כדי להמשיך.";
    if (!skipEmailPassword) {
      if (!email.trim()) return "יש להזין כתובת אימייל.";
      if (!validateEmail(email)) return "כתובת אימייל לא תקינה.";
      if (password.length < 6) return "הסיסמה חייבת להכיל לפחות 6 תווים.";
      if (password !== passwordConfirm) return "הסיסמאות אינן תואמות.";
    }
    return null;
  };
  ...
  const finalizeOnboarding = async () => {
    ...
    const alreadySignedIn = Boolean(preSession?.user);

    const err = validateFinalize(alreadySignedIn);
    if (err) {
      setErrorBanner(err);
      return;
    }
    ...
    try {
      const existingSession = preSession;

      if (existingSession?.user) {
        await resolveRoleForUser(supabase, existingSession.user, "sitter", full_name.trim() || null);
      } else {
        ...
        if (signErr) {
          setErrorBanner(signErr.message || "ההרשמה נכשלה.");
          return;
        }

        if (!signData.user) {
          setErrorBanner("לא נוצר משתמש. נסו שוב.");
          return;
        }

        if (!signData.session) {
          setErrorBanner(
            "נשלח מייל אימות. לאחר לחיצה על הקישור במייל והתחברות, תוכלו להשלים את הפרופיל מהתחברות."
          );
          return;
        }

        await resolveRoleForUser(supabase, signData.user, "sitter", full_name.trim() || null);
      }
      ...
      if (!res.ok) {
        setErrorBanner(json.error ?? "שגיאת שמירת פרופיל.");
        return;
      }
    ...
    <main className="mx-auto max-w-md space-y-6 px-4 py-6 pb-28" dir="rtl">
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-xs font-medium text-slate-500">
          ...
          <span>
            {step === 1 ? "חשיפה ציבורית" : step === 2 ? "מידע חסוי" : "חשבון והצהרה"}
          </span>
        </div>
      ...
      {step === 3 ? (
        <section className="space-y-4 rounded-3xl border border-navy-header/15 bg-white p-5 shadow-soft">
          <h2 className="text-lg font-bold text-[#001F3F]">שלב 3 — חשבון והצהרה</h2>
          <p className="text-sm leading-relaxed text-slate-600">
