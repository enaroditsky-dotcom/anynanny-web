...
function SitterMessagesInner() {

  const router = useRouter();

  const searchParams = useSearchParams();

  const parentIdFromQuery = searchParams.get("parentId")?.trim() ?? "";
  ...
  const openChatWithParent = useCallback(
    ...
    async (parentId: string) => {
      ...
      const { bookingId, error } = await findChatBookingForParentSitter(supabase, parentId, auth.userId);
      ...
      router.replace(`/sitter/chat/${encodeURIComponent(bookingId)}`);
  ...
  useEffect(() => {
    ...
    if (isLoading || !signedIn || effectiveRole !== "sitter") return;

    if (parentIdFromQuery) {

      void openChatWithParent(parentIdFromQuery);

    }

  }, [isLoading, signedIn, effectiveRole, parentIdFromQuery, openChatWithParent]);



  if (parentIdFromQuery && !deepLinkError) {
  ...
  return (
    ...
    <>
      ...
      {!parentIdFromQuery ? <SitterBookingChatInbox /> : null}
